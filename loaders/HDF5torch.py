...
class HDF5generator():
    ...
    def __iter__(self):
       while True:
          ...
          for key in self.data:
            batch[key]=self.data[key][self.index:self.batch_size + self.index].cuda() # get a batch
    ...
    def preproc(self, data):
       for key in data:
           ...
           data[key] = torch.autograd.Variable(data[key].cuda(), requires_grad= requires_grad)
